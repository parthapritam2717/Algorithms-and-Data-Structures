...
int constructSTUtil(int arr[], int starts, int ends, int *st, int scurrent){
    ...
    (st)[scurrent]=constructSTUtil(arr,starts,mid,st,2*scurrent+1)+constructSTUtil(arr,mid+1,ends,st,scurrent*2+2);
    return (st)[scurrent];
}
...
int main(){
            ...
            cout<<getSum(segmentTree,n,0,2);
            // now lets update the values of the
            nl();
            updateValue(list1,segmentTree,n,0,1,10);
            // now lets recheck the sum
            cout<<getSum(segmentTree,n,0,1);
             delete [] segmentTree;
    return 0;
